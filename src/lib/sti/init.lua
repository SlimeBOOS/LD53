...
function Map:init(path, plugins, ox, oy)
	...
	for i, tileset in ipairs(self.tilesets) do
		...
		if lg.isCreated then
			local formatted_path = utils.format_path(tileset.image)
...
function Map:setTiles(index, tileset)
	...
	for idx, tile in pairs(tileset.tiles) do
		...
		local x = (idx - 1) % (imageW / tileW)
		local y = math.floor((idx - 1) / (imageW / tileW))
		local quadX = x * tileW + margin + x * spacing
		local quadY = y * tileH + margin + y * spacing
